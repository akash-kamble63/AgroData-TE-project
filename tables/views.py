...
from .forms import Cropform, Plotform, Soilform, Issueform, Toolform, plantationandharvestingform
# Diseaseform
from .models import Crop, Plot, Soil, Issue, Tool, plantationandharvesting
# Disease
...
def crudplantation(request):

    plantation = plantationandharvesting.objects.filter(user = request.user)
    if request.method == "POST":
        fm = plantationandharvestingform(request.POST)
        if fm.is_valid():
            tab = plantationandharvesting(user = request.user,
            location = fm.cleaned_data['location'],
            plotid = fm.cleaned_data['plotid'],
            cropid = fm.cleaned_data['cropid'],
            dateofpreparation = fm.cleaned_data['dateofpreparation'],
            dateofplantation = fm.cleaned_data['dateofplantation'],
            startofharvesting = fm.cleaned_data['startofharvesting'],
            endofharvesting = fm.cleaned_data['endofharvesting'],
            currentharvest = fm.cleaned_data['currentharvest'],
            previousharvest = fm.cleaned_data['previousharvest'],
            accumulatedharvest = fm.cleaned_data['accumulatedharvest'])
            tab.save()
            fm = plantationandharvestingform()
    else:
        fm = plantationandharvestingform()

    return render(request, 'tables/crudplantationandharvesting.html',{'crop':plantation, 'form':fm})

def editplantationandharvesting(request,id):

    if request.method == 'POST':
        obj = plantationandharvesting.objects.get(id = id)
        obj.location = request.POST['location']
        obj.plotid = request.POST['plotid']
        obj.cropid = request.POST['cropid']
        obj.dateofpreparation = request.POST['dateofpreparation']
        obj.dateofplantation = request.POST['dateofplantation']
        obj.startofharvesting = request.POST['startofharvesting']
        obj.endofharvesting = request.POST['endofharvesting']
        obj.currentharvest = request.POST['currentharvest']
        obj.previousharvest = request.POST['previousharvest']
        obj.accumulatedharvest = request.POST['accumulatedharvest']

        obj.save()        
        return HttpResponseRedirect('/crudplantationandharvesting/')
    else:
        pl = plantationandharvesting.objects.get(id = id)
        return render(request, 'tables/editplantationandharvesting.html', {'form':pl})

def deleteplantationandharvesting(request,id):
    if request.method == 'POST':
        pl = plantationandharvesting.objects.get(id = id)
        pl.delete()
        return HttpResponseRedirect('/crudplantationandharvesting/')
